...
def pre_process(orig_img):
    imshow = cv2.imshow('orig_img', orig_img)

    # HSV图像
    hsv_img = cv2.cvtColor(orig_img, cv2.COLOR_BGR2HSV)
    h, s, v = hsv_img[:, :, 0], hsv_img[:, :, 1], hsv_img[:, :, 2]
    # 车牌背景为黄色或蓝色是，查找HSV颜色范围，黄色色调区间[26，34],蓝色色调区间:[100,124]
    color_img = (((h > 20) & (h < 40)) | ((h > 100) & (h < 124))) & (s > 70) & (v > 70)
    ...
    # blue_img = (((h > 26) & (h < 34)) | ((h > 100) & (h < 124))) & (s > 43) & (v > 46)
    # blue_img = blue_img.astype('float32')

    mix_img = np.multiply(sobel_img, color_img)
    ...
    dilate_img = cv2.morphologyEx(binary_img, cv2.MORPH_DILATE, kernel)
    close_img = cv2.morphologyEx(dilate_img, cv2.MORPH_ERODE, kernel)
    # close_img = cv2.morphologyEx(binary_img, cv2.MORPH_CLOSE, kernel)
    cv2.imshow('close', close_img)
    cv2.imwrite('C:\\Users\\Liu\\Desktop\\opencv_output\\12\\close.jpg', close_img)
    return close_img
...
def verify_color(rotate_rect, src_image):
    ...
    for i in range(rand_seed_num):
        ...
        # 限制随机种子必须是车牌背景色
        if (((h[row, col] > 20) & (h[row, col] < 40)) | ((h[row, col] > 100) & (h[row, col] < 124))) & (
...
def locate_carPlate(orig_img, pred_image):
    ...
    for i, contour in enumerate(contours):
        ...
        if verify_scale(rotate_rect):
            ...
            cv2.imwrite('C:\\Users\\Liu\\Desktop\\opencv_output\\12\\opencv_' + str(i) + '.jpg', car_plate)
            # ========================调试看效果========================#
...
def get_chars(car_plate):
    ...
    cv2.imwrite('C:\\Users\\Liu\\Desktop\\opencv_output\\12\\car.jpg', car_plate)
    cv2.imwrite('C:\\Users\\Liu\\Desktop\\opencv_output\\12\\plate.jpg', plates)
    ...
    for i, addr in enumerate(char_addr_list):
        ...
        cv2.imshow('char_' + str(i), char_img)
        cv2.imwrite('C:\\Users\\Liu\\Desktop\\opencv_output\\12\\char_' + str(i) + '.jpg', char_img)
    return char_imgs


def extract_char(car_plate):
    hsv_img = cv2.cvtColor(car_plate, cv2.COLOR_BGR2HSV)
    h = hsv_img[:, :, 0]
    (mean, stddv) = cv2.meanStdDev(h)
    if mean < 40:
        ...
        cv2.bitwise_not(binary_plate, binary_plate);
    else:
        gray_plate = cv2.cvtColor(car_plate, cv2.COLOR_BGR2GRAY)
        ret, binary_plate = cv2.threshold(gray_plate, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
...
if __name__ == '__main__':
    ...
    char_w, char_h = 20, 20
    plate_model_path = os.path.join(cur_dir, 'carIdentityData\\model\\plate_recognize\\model.ckpt-530.meta')
    char_model_path = os.path.join(cur_dir, 'carIdentityData\\model\\char_recognize\\model.ckpt-570.meta')
    img = cv2.imread("C:\\Users\\Liu\\Desktop\\CarPlateIdentity-master\\carIdentityData\\pictures\\12.jpg")
